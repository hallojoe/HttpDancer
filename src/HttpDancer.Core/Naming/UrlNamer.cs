...
public class UrlNamer : IUrlNamer
{
    private readonly IOptions<UrlNamingOptions> _urlNamingOptions;
    private readonly IPathSegmentFilter _pathSegmentFilter;
    private readonly IPathSegmentNormalizer _pathSegmentNormalizer;
    ...
    {
        _urlNamingOptions = urlNamingOptions;
        _pathSegmentFilter = pathSegmentFilter;
        _pathSegmentNormalizer = pathSegmentNormalizer;
        _queryStringProcessor = queryStringProcessor;
    }

    public UrlNamingOptions Options => _urlNamingOptions.Value;
    ...
    public UrlNamingResult GetNameAndPath(Uri uri)
    {
        ...
        var sanitizedPath = _pathSegmentFilter.SanitizePath(adjustedPath, currentOptions.ExcludedPathSegments ?? []);
        ...
        var normalizedName = _pathSegmentNormalizer.NormalizeSegment(extensionAdjustedName, currentOptions);
        ...
        var queryProcessing = _queryStringProcessor.Process(uri, currentOptions);
        ...
        UrlNamingOptions options)
    {
        ...
        if (segments.Count > 0)
        {
            ...
            {
                pathSegments.Add(_pathSegmentNormalizer.NormalizeSegment(segments[index], options));
            }
        }

        if (options.PathComponents.HasFlag(PathComponents.QueryHash) && !string.IsNullOrEmpty(queryProcessingResult.QueryHash))
        {
            pathSegments.Add(_pathSegmentNormalizer.NormalizeSegment(queryProcessingResult.QueryHash!, options));
        }

        return pathSegments;
    }

    private static string NormalizePath(IEnumerable<string> segments, UrlNamingOptions options)
    {
